...
from torch_spyre._inductor.pass_utils import (
    ...
    compute_fractional_view_relayout_target,
    device_coordinates,
...
def test_nonunit_three_digits_preserve_addresses(step, start):
    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    index = step * k + start
    count = len(range(start, step * 64, step))
    synthetic = itertools.count()
    terms = normalize_coordinates(
        {k: count, lane: 64},
        [step, 8, 8, 64],
        [sympy.floor(index / 64), sympy.Mod(sympy.floor(index / 8), 8),
         sympy.Mod(index, 8), lane],
        lambda: sympy.Symbol(f"gap{next(synthetic)}"), compare_value=int,
    )
    flat = sympy.S.Zero
    stride = 1
    for term in reversed(terms):
        coord = term.offset
        if term.var is not None:
            coord += sympy.floor(term.num * sympy.Mod(term.var, term.mod) / term.den)
        flat += stride * coord
        stride *= term.dim_size
    assert stride == step * 64 * 64
    for value in range(count):
        values = dict.fromkeys(flat.free_symbols, 0)
        values.update({k: value, lane: 17})
        assert int(flat.subs(values)) == (step * value + start) * 64 + 17


@pytest.mark.parametrize("step", [3, 5, 7])
def test_nonunit_periodic_digits_preserve_addresses(step):
    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    # Keep the inner digit unevaluated below: eager nested-Mod simplification
    # in the installed SymPy can alter the expression before normalization.
    index = step * sympy.Mod(k, 64)
    synthetic = itertools.count()
    terms = normalize_coordinates(
        {k: 128, lane: 64}, [step, 8, 8, 64],
        [sympy.floor(index / 64), sympy.Mod(sympy.floor(index / 8), 8),
         sympy.Mod(index, 8, evaluate=False), lane],
        lambda: sympy.Symbol(f"gap{next(synthetic)}"), compare_value=int,
    )
    flat = sympy.S.Zero
    stride = 1
    ...
    for value in range(128):
        values = dict.fromkeys(flat.free_symbols, 0)
        values.update({k: value, lane: 17})
        assert int(flat.subs(values)) == step * (value % 64) * 64 + 17


@pytest.mark.parametrize("step", [3, 5, 7])
def test_periodic_coordinate_construction_preserves_addresses(step):
    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    coords = compute_coordinates(
        [step, 8, 8, 64], [4096, 512, 64, 1], {k: 128, lane: 64},
        64 * step * sympy.Mod(k, 64) + lane,
    )
    address = sum(c * stride for c, stride in zip(coords, [4096, 512, 64, 1]))
    for value in range(128):
        assert int(address.subs({k: value, lane: 17})) == 64 * step * (value % 64) + 17


@pytest.mark.parametrize("step", [3, 5, 7])
def test_periodic_view_copies_separated_digits(step):
    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    source = SpyreTensorLayout([step, 8, 8, 64], torch.float16)
    output = SpyreTensorLayout([128, 64], torch.float16)
    read = MemoryDep(
        "input", 64 * step * sympy.Mod(k, 64) + lane, (k, lane), (128, 64)
    )
    write = MemoryDep("output", 64 * k + lane, (k, lane), (128, 64))
    target = compute_fractional_view_relayout_target(source, read, output, write)
    assert target is not None
    assert list(target.stride_map[:-1]) == sorted(source.stride_map[:-1], reverse=True)
    assert target.stride_map[-1] == source.stride_map[-1]
    assert sympy.prod(target.device_size) == sympy.prod(source.device_size)


def test_offset_carries_before_fractional_normalization():
    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    coords = compute_coordinates(
        [2, 64, 64], [4096, 64, 1], {k: 13, lane: 64},
        64 * (5 * k + 63) + lane,
    )
    for value in range(13):
        evaluated = [int(coord.subs({k: value, lane: 17})) for coord in coords]
        assert evaluated == [(5 * value + 63) // 64, (5 * value + 63) % 64, 17]


@pytest.mark.parametrize("step", [3, 5])
def test_fractional_padding_plan_preserves_inner_radix(step):
    from torch_spyre._inductor.views import _coalesce_quotient_remainder_dims

    k, lane = sympy.symbols("k lane", integer=True, nonnegative=True)
    size = [2, 64, 64]
    coordinates = [sympy.floor(step * k / 64), sympy.Mod(step * k, 64), lane]
    padding = {}
    merged_size, merged_coordinates = _coalesce_quotient_remainder_dims(
        size, coordinates, padding_out=padding
    )
    assert padding == {0: step}
    assert merged_size == [step * 64, 64]
    assert merged_coordinates == [step * k, lane]
    # Planning cannot resize the existing input allocation.
    assert size == [2, 64, 64]
    assert coordinates == [sympy.floor(step * k / 64), sympy.Mod(step * k, 64), lane]
